...
class Option extends BaseModel implements HasMedia
{
    ...
    {
        if ($this->attributes['no_components']) {
            return true;
        }
    ...
    public function retire(bool $status = true): bool
    {
        ...
        if ($this->formElementItems()->count() > 0) {
            return false;
        }
    ...
    {
        return self::where('option_name', $this->attributes['option_name'])
