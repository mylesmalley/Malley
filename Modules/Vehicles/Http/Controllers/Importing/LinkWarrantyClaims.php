...
class LinkWarrantyClaims extends Controller
{
    ...
    public function fix()
    {
        ...
        $badVins = WarrantyClaim::where('vehicle_id', null)
                ->where('vin','!=', null)
                ->pluck('vin');
        ...
        {
            WarrantyClaim::where('vin', $old)->update(['vin'=>$new] );
...
//        });
        ...
        foreach ($vehicles as $id => $vin) {
            ...
            {
                WarrantyClaim::where('vin', '=', $vin)->update(['vehicle_id' => $id]);
                echo "done {$vin}\r\n";
            }
            else
            {
        ...
        echo "couldn't match up " . WarrantyClaim::where('vehicle_id', '=', null )->count();
