<!DOCTYPE html>
<html lang="en">

<head>

    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="">
    <meta name="author" content="">

    <title>Malley Industries Warranty Registration Form</title>

    <!-- Bootstrap core CSS -->
    <link href="{{ url('shared/css/bootstrap.min.css') }}" rel="stylesheet">

    <style>
        .fr {
            color: #274e72;
        }
    </style>
</head>

<body>
<br>
<br>


<div class="container text-center">
    <div class="col-md-8">
    <img src="data:image/png;base64,"
         height="47"
         style="filter: invert(100%) ;"
         alt=""></div>
</div>



<!-- Page Content -->
<div class="container">




    <div class="row">
        <div class="col-lg-8 text-center">
            <h1>Warranty Registration Form<br>
                <span class="fr">Formulaire d'inscription de garantie</span>
            </h1>
        </div>
    </div>



    <div class="row">
        <div class="col-md-4">
            <p>Thank you for purchasing a vehicle or conversion through Malley Industries Inc. We sincerely appreciate your business</p>
            <p>Please take a moment to register your warranty.</p>
            <p>You can use your smartphone's camera feature to scan the code below. Alternatively, go to www.malleyindustries.com/warranty. You will be asked to provide your vehicle's VIN, which is
                <b><u>{{ $vehicle->vin }}</u></b> as well as proof of purchase.</p>
        </div>

        <div class="col-md-4">
            <p class="fr">

                Merci pour votre achat d'une conversion Malley Industries Inc. Nous appr&eacute;cions tous nos
                    clients.</p>


            <p class="fr">S'il vous plait prendre un moment pour enregistrer votre garantie.</p>


            <p class="fr">Vous pouvez utiliser la cam&eacute;ra sur votre t&eacute;l&eacute;phone intelligent pour lire le code
                ci-bas.
                Vous pouvez aussi suivre le liens suivant afin de remplir le formulaire.
                www.malleyindustries.com/warranty vous serez demander d'inscrire votre VIN qui est
                {{ $vehicle->vin }}, ainsi que attacher une preuve d’achat.</p>




            </p>
        </div>
    </div>




    <div class="row text-center">
        <div class="col-md-8">
        <img src="data:image/png;base64,{!! DNS2D::getBarcodePNG( "https://blueprint.malleyindustries.com/warranty/register/".enc($vehicle->id) , "QRCODE", 8,8) !!}"
             width="150"
             height="150"
             alt="barcode"   />
        </div>

    </div>


</div>



</body>

</html>




