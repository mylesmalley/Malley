...
//        dd( $bin_name );
        ...
        foreach( $pages as $page )
        {
            ...
            for( $i = 0; $i < count($page); $i++)
            {
                ...
                $this->extracted($inventory, $pdf, $i, $page[$i], $suppliers[$page[$i]->supplier]);
...
//
     ...
     * @param $suppliers
     */
    private function extracted(Inventory $inventory, Fpdf &$pdf, int $i, $page, $suppliers): void
    {
...
//                // stub print area
        ...
        $ticket_number_text = $page->line_status !== "Needs Recount"
            ? "#" . str_pad($page->ticket_number, 4, "0", STR_PAD_LEFT)
            : "#" . str_pad($page->ticket_number, 4, "0", STR_PAD_LEFT) . 'R';

        $part_text = "PART: " . trim($page->stock_code) ?? 'STOCK CODE';
        $bin_text = trim("AREA: " . $page->group . " BIN: " . $page->bin ?? 'BIN');
        ...
        $pdf->Cell(3, 0.18, $page->description_1, 0, 2);
        $pdf->Cell(3, 0.18, $page->description_2, 0, 2);
        $pdf->Ln();
        $pdf->SetX(0.25);
        $pdf->Cell(3, 0.18, "Supplier: ", 0, '');
        $pdf->SetX(0.25 + $description_text_length);

        $pdf->Cell(3, 0.18, $suppliers ?? '', 0, 2);

        $cat = isset($page->catalogue) ? "Supplier# " . $page->catalogue : '';
        ...
        $unitExploded = $this->units[$page->unit_of_measure] ?? "Each";
        ...
        $part_text = trim($page->stock_code) ?? 'STOCK CODE';
        $bin_text = trim("BIN: " . $page->bin ?? 'BIN');
...
//                $pdf->Cell( $ticket_number_length ,0.25, $ticket_number_text ,1 );//, 0, '', true);
        ...
        $pdf->Cell(2, .18, $ticket_number_text . "       QTY________" . $page->unit_of_measure, 0, 2);
        $pdf->SetX($stub_start_x);
