<!DOCTYPE html>
<html lang="en">

<head>

	<meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
	<meta name="description" content="">
	<meta name="author" content="">
    <meta name="csrf-token" content="{{ csrf_token() }}">

	<title>Blueprint Bugs and Feedback</title>

	<!-- Bootstrap core CSS -->
{{--	<link href="{{ url('shared/css/bootstrap.min.css') }}" rel="stylesheet">--}}
    <link href='{{ mix('css/homepage.css') }}' rel='stylesheet'></link>

    <link href='{{ url('shared/datepicker/datepicker.css') }}' rel='stylesheet'></link>
    @includeIf('homepage::googleAnalytics')

</head>

<body>

@includeIf('homepage::malleyMenu')
<br>
<h4 class="text-secondary text-center">Bug Reports &amp; Engineering Tasks</h4>

<!-- Navigation -->

{{--<nav class="navbar navbar-expand-lg navbar-dark bg-dark static-top">--}}
{{--	<div class="container">--}}
{{--			<img src="data:image/png;base64,"--}}
{{--			     style="height:30px;"--}}
{{--			     alt="">--}}

{{--		<div class="collapse navbar-collapse" id="navbarResponsive">--}}
{{--			<ul class="navbar-nav ml-auto">--}}




{{--				@if (  $malley ?? true )--}}
{{--                    <li class="nav-item dropdown">--}}
{{--                        <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">--}}
{{--                            New--}}
{{--                        </a>--}}
{{--                        <div class="dropdown-menu" aria-labelledby="navbarDropdown">--}}
{{--                            <a class="dropdown-item" href="https://blueprint.malleyindustries.com/bugs/create">Blueprint / Index Bug</a>--}}
{{--                            <a class="dropdown-item" href="https://index.malleyindustries.com/bugs/engineering">Engineering Task</a>--}}
{{--                        </div>--}}
{{--                    </li>--}}


{{--                    <li class="nav-item dropdown">--}}
{{--                        <a class="nav-link dropdown-toggle" href="#" id="navbarDropdownTasks" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">--}}
{{--                            Tasks--}}
{{--                        </a>--}}
{{--                        <div class="dropdown-menu" aria-labelledby="navbarDropdownTasks">--}}
{{--                            <a class="dropdown-item" href="https://index.malleyindustries.com/bugs/all/engineering">Open Engineering Projects</a>--}}
{{--                            <a class="dropdown-item" href="https://index.malleyindustries.com/bugs/all/blueprint">Open Blueprint Issues</a>--}}
{{--                            <a class="dropdown-item" href="https://index.malleyindustries.com/bugs/all">Most Recent</a>--}}
{{--                            <a class="dropdown-item" href="https://index.malleyindustries.com/bugs/all/open">Open Tasks</a>--}}
{{--                            <a class="dropdown-item" href="https://index.malleyindustries.com/bugs/all/unassigned">Unassigned Tasks</a>--}}
{{--                        </div>--}}
{{--                    </li>--}}



{{--                    <li class="nav-item">--}}
{{--					<a class="nav-link active" href="http://index.malleyindustries.com/bugs/all">All Tasks</a>--}}
{{--				</li>--}}

{{--                    <li class="nav-item">--}}
{{--                        <a class="nav-link active" href="https://index.malleyindustries.com/bugs/user/{{ Auth::user()->id }}">My Tasks</a>--}}
{{--                    </li>--}}
{{--                @endif--}}
{{--			</ul>--}}
{{--		</div>--}}
{{--	</div>--}}
{{--</nav>--}}

<!-- Page Content -->
<div class="container">
{{--	<div class="row">--}}
{{--		<div class="col-lg-12 text-center">--}}
{{--			<h1 class="mt-5">Bug Report &amp; Feedback </h1>--}}
{{--		</div>--}}
{{--	</div>--}}
	@yield('content')

</div>

<!-- Bootstrap core JavaScript -->

<script src='{{ mix('js/homepage.js') }}'></script>

{{--<script src='{{ url('shared/js/jquery.min.js') }}'></script>--}}
{{--<script src='{{ url('shared/js/bootstrap.min.js') }}'></script>--}}


	@yield('scripts')

</body>

</html>
