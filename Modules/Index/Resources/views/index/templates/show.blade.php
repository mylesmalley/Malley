...
@section('content')
    ...
    <form action="{{ url( '/basevan/'.$basevan->id.'/templates/'.$template->id ) }}" method="POST">
        {{ method_field("PATCH") }}
        {{ csrf_field() }}
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                ...
                                <input type="text"
                                       id="name"
                                       class="form-control"
                                       value="{{ old('title') ?? str_replace( '*','', $template->name ) }}"
                                       name="name">
                            </div>
                            <div class="col-md-12">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-2">
                                ...
                                <select name="visibility" class="form-control" id="visibility">
                                    @foreach(['1'=>"Visible",'0'=>'Hidden'] as $k => $v)
                                        <option
                                                @if ( $k === old('visibility') || $template->visibility === $k)
                                                selected
                                                @endif
                                                value="{{ $k }}">{{ $v }}</option>
                                    @endforeach
                            ...
                            <div class="col-md-2">
                                ...
                                <select name="sales_drawing"  class="form-control"  id="sales_drawing">
                                    ...
                                    @foreach(['1'=>"Yes",'0'=>'No'] as $k => $v)
                                        <option
                                                @if ( $k === old('sales_drawing') || $template->sales_drawing === $k)
                                                selected
                                                @endif
                                                value="{{ $k }}">{{ $v }}</option>
                                    @endforeach
                            ...
                            <div class="col-md-2">
                                ...
                                <select name="production_drawing"  class="form-control"  id="production_drawing">
                                    ...
                                    @foreach(['1'=>"Yes",'0'=>'No'] as $k => $v)
                                        <option
                                                @if ( $k == old('production_drawing') || $template->production_drawing == $k)
                                                selected
                                                @endif
                                                value="{{ $k }}">{{ $v }}</option>
                                    @endforeach
