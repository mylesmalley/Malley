...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <h1> {{ $element->label }} Checkbox List ( Yes or No )
    ...
    @includeIf('index::index.forms.elements.shared.element_details_form',['target'=>'checkbox'])
                        ...
                        @foreach($element->items->sortBy('position') as $item)
                            <tr>
                                ...
                                <td>
                                    <form
                                            method="POSt"
                                            action="{{ url('/index/forms/checkbox/') }}"
                                            class="form-inline">
                                        {{ csrf_field() }}
                                        {{ method_field('PATCH') }}
                            ...
                            <form>
                                ...
                                <script>
                                    ...
                                    function buildUrl(  )
                                    {
                                        ...
                                        if ( filter === '0')
                                        {
                                            window.location.href = `{{ url("index/forms/checkbox/{$element->id}") }}`;
                                        }
                                        else
                                        {
                                            window.location.href = `{{ url("index/forms/checkbox/{$element->id}") }}?filter=${filter}`;
                                        }
                                    }
